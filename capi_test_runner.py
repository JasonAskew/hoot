...
class CAPITestRunner:
    ...
    def run_test_case(self, test_case: Dict) -> Dict:
        ...
        if actual_segments and not segment_matched:
            ...
            error_message = f"Segment mismatch - Expected: {segment}, Got: {', '.join(actual_segments)}"
        elif actual_intent and not intent_matched:
            status = 'INTENT_MISMATCH'  # Wrong intent recognized
            error_message = f"Intent mismatch - Expected: {intent}, Got: {actual_intent}"
            ...
            error_message = "High text similarity but not exact match"
        elif comparison['text_similarity'] > 0.6:
            status = 'CONTENT_MISMATCH'  # Some similarity but not exact
            error_message = f"Content mismatch (similarity: {comparison['text_similarity']:.2f})"
        else:
            status = 'FAILED_RESPONSE'  # Poor match
            # Truncate responses for readability
            expected_text = expected_content['text'][:100] + '...' if len(expected_content['text']) > 100 else expected_content['text']
            actual_text = actual_content['text'][:100] + '...' if len(actual_content['text']) > 100 else actual_content['text']
            error_message = f"Response mismatch - Expected: '{expected_text}', Got: '{actual_text}'"
    ...
    def run_conversation_flow_test(self, test_case: Dict) -> Dict:
        ...
        # For now, just use the expected response as text directly
        ...
        if expected_clarify_response and actual_content_1.get('text'):
            # Use compare_responses to check match
            comparison = self.compare_responses(
                actual_content_1,
                {'text': expected_clarify_response, 'buttons': expected_clarify_buttons}
            )
            turn_1_match = comparison['text_similarity'] > 0.8
            
        if not turn_1_match and actual_intent_1 != intent:
            # Truncate responses for readability
            expected_text = expected_clarify_response[:100] + '...' if len(expected_clarify_response) > 100 else expected_clarify_response
            actual_text = actual_content_1.get('text', '')[:100] + '...' if len(actual_content_1.get('text', '')) > 100 else actual_content_1.get('text', '')
            return {
                'test_case_number': test_case.get('test_case_number', ''),
                'segment': segment,
                'intent': intent,
                ...
                'error': f"Turn 1 clarification response mismatch - Expected: '{expected_text}', Got: '{actual_text}'",
                'test_type': 'conversation_flow',
        ...
        if total_turns >= 2 and turn_2_input:
            ...
            if comparison['overall_match']:
                ...
                status = 'FAILED_RESPONSE'
                # Truncate responses for readability
                expected_text = turn_2_expected_content[:100] + '...' if len(turn_2_expected_content) > 100 else turn_2_expected_content
                actual_text = actual_content_2.get('text', '')[:100] + '...' if len(actual_content_2.get('text', '')) > 100 else actual_content_2.get('text', '')
                error_message = f"Turn 2 response mismatch - Expected: '{expected_text}', Got: '{actual_text}'"
        ...
        if not turn_1_match:
            expected_text = expected_clarify_response[:100] + '...' if len(expected_clarify_response) > 100 else expected_clarify_response
            actual_text = actual_content_1.get('text', '')[:100] + '...' if len(actual_content_1.get('text', '')) > 100 else actual_content_1.get('text', '')
            result['error'] = f"Single-turn response mismatch - Expected: '{expected_text}', Got: '{actual_text}'"
            
        return result


    def run_intent_navigation_test(self, test_case: Dict) -> Dict:
        """Run an intent navigation test (click button to navigate to another intent)."""
        segment = test_case['segment']
        source_intent = test_case.get('source_intent', test_case.get('intent', ''))
        target_intent = test_case.get('target_intent', test_case.get('expected_intent', ''))
        example_trigger = test_case['example_trigger']
        
        # Turn 1 expected response (the source intent response)
        turn_1_expected_response = test_case.get('expected_response', '')
        turn_1_expected_buttons = test_case.get('expected_buttons', '')
        
        # Turn 2 fields (navigation)
        turn_2_input = test_case.get('turn_2_input', '')  # The button text to click
        turn_2_expected_response = test_case.get('turn_2_expected_response', '')
        user_action = test_case.get('user_action', '')
        
        logger.info(f"Testing intent navigation: {segment} | {source_intent} → {target_intent}")
        logger.info(f"  Turn 1: {example_trigger[:30]}... → expecting {source_intent} response")
        logger.info(f"  Turn 2: Clicking '{turn_2_input}' → expecting {target_intent} response")
        ...
        if not session_id:
            return {
                ...
                'example_trigger': example_trigger,
                'expected_response': turn_1_expected_response,
                'actual_response': None,
                'session_id': None,
                'status': 'FAILED_SESSION',
                'error': 'Failed to start session',
        ...
        actual_content_1 = self.extract_response_content(response_data_1)
        actual_intent_1 = self.extract_intent_name(response_data_1)
        actual_segments_1 = self.extract_segment_names(response_data_1)
        
        # Parse expected response for turn 1
        expected_content_1 = self.parse_expected_response(turn_1_expected_response, turn_1_expected_buttons)
        
        # Compare turn 1 response
        comparison_1 = self.compare_responses(actual_content_1, expected_content_1)
        
        # Check if turn 1 matches expected source intent response
        if not comparison_1['overall_match'] and comparison_1['text_similarity'] < 0.8:
            # Turn 1 failed - return early
            expected_text = expected_content_1['text'][:100] + '...' if len(expected_content_1['text']) > 100 else expected_content_1['text']
            actual_text = actual_content_1['text'][:100] + '...' if len(actual_content_1['text']) > 100 else actual_content_1['text']
            return {
                'test_case_number': test_case.get('test_case_number', ''),
                'segment': segment,
                'actual_segments': actual_segments_1,
                'intent': source_intent,
                'actual_intent': actual_intent_1,
                'example_trigger': example_trigger,
                'expected_response': turn_1_expected_response,
                'actual_response': actual_content_1['text'],
                'actual_buttons': actual_content_1['buttons'],
                'expected_buttons': expected_content_1['buttons'],
                'session_id': session_id,
                'status': 'FAILED_RESPONSE',
                'error': f"Turn 1 response mismatch - Expected: '{expected_text}', Got: '{actual_text}'",
                'test_type': 'intent_navigation',
                'comparison': comparison_1,
                'timestamp': datetime.now().isoformat()
            }
        
        # TURN 2: Send button text to navigate
        if turn_2_input:
            logger.info(f"  Sending turn 2 navigation: {turn_2_input}")
            ...
            if not response_data_2:
                return {
                    ...
                    'session_id': session_id,
                    'status': 'FAILED_MESSAGE',
                    'error': 'Failed to send turn 2 navigation',
                    'test_type': 'intent_navigation'
                }
            
            # Extract turn 2 response
            actual_content_2 = self.extract_response_content(response_data_2)
            actual_intent_2 = self.extract_intent_name(response_data_2)
            actual_segments_2 = self.extract_segment_names(response_data_2)
            
            # Parse expected response for turn 2
            expected_content_2 = self.parse_expected_response(turn_2_expected_response, '')
            
            # Compare turn 2 response
            comparison_2 = self.compare_responses(actual_content_2, expected_content_2)
            
            # Check if we navigated to the correct intent
            intent_matched = actual_intent_2 == target_intent if actual_intent_2 else True
            
            # Determine final status
            if not intent_matched:
                status = 'INTENT_MISMATCH'
                error_message = f"Navigation failed - Expected intent: {target_intent}, Got: {actual_intent_2}"
            elif comparison_2['overall_match']:
                status = 'PASSED'
                error_message = None
            elif comparison_2['text_similarity'] > 0.8:
                status = 'PARTIAL_PASS'
                error_message = "High text similarity but not exact match"
            else:
                status = 'FAILED_RESPONSE'
                expected_text = expected_content_2['text'][:100] + '...' if len(expected_content_2['text']) > 100 else expected_content_2['text']
                actual_text = actual_content_2['text'][:100] + '...' if len(actual_content_2['text']) > 100 else actual_content_2['text']
                error_message = f"Turn 2 response mismatch - Expected: '{expected_text}', Got: '{actual_text}'"
            
            # Return results showing BOTH turns
            result = {
                'test_case_number': test_case.get('test_case_number', ''),
                'segment': segment,
                'actual_segments': actual_segments_2,
                'intent': source_intent,  # Keep source intent as primary
                'actual_intent': actual_intent_1,  # Turn 1 actual intent
                'example_trigger': example_trigger,
                'expected_response': turn_1_expected_response,  # Turn 1 expected
                'actual_response': actual_content_1['text'],  # Turn 1 actual
                'actual_buttons': actual_content_1.get('buttons', []),
                'expected_buttons': expected_content_1.get('buttons', []),
                'session_id': session_id,
                'status': status,
                'comparison': comparison_1,  # Turn 1 comparison
                'error': error_message,
                'test_type': 'intent_navigation',
                # Turn 2 specific fields
                'turn_2_input': turn_2_input,
                'turn_2_expected_response': turn_2_expected_response,
                'turn_2_actual_response': actual_content_2['text'],
                'turn_2_expected_buttons': expected_content_2.get('buttons', []),
                'turn_2_actual_buttons': actual_content_2.get('buttons', []),
                'turn_2_intent': target_intent,
                'turn_2_actual_intent': actual_intent_2,
                'turn_2_comparison': comparison_2,
                'full_response': response_data_2,
                'timestamp': datetime.now().isoformat()
            }
        ...
        # If no turn 2, return turn 1 results
        return {
            'test_case_number': test_case.get('test_case_number', ''),
            'segment': segment,
            'actual_segments': actual_segments_1,
            ...
            'expected_buttons': expected_content_1['buttons'],
            'session_id': session_id,
            'status': 'PASSED' if comparison_1['overall_match'] else 'FAILED_RESPONSE',
            'comparison': comparison_1,
            'test_type': 'intent_navigation',
            'full_response': response_data_1,
            'timestamp': datetime.now().isoformat()
            ...
            if self.format_type == "hoot":
                ...
                for row in csv_rows:
                    ...
                    try:
                        ...
                        elif test_source == 'intent_navigation' or test_type == 'intent_navigation':
                            result = self.run_intent_navigation_test(row)
                        else:
        ...
        try:
            with open(filename, 'w', newline='', encoding='utf-8') as f:
                ...
                if self.format_type == "hoot":
                    ...
                    fieldnames = [
                        'test_case_number', 'test_number', 'segment', 'actual_segments', 'intent', 'actual_intent', 'example_trigger', 
                        'status', 'text_match', 'text_similarity', 'buttons_match',
                        'expected_response', 'actual_response', 
                        'expected_buttons', 'actual_buttons',
                        'turn_2_input', 'turn_2_expected_response', 'turn_2_actual_response',
                        'turn_2_intent', 'turn_2_actual_intent',
                        'session_id', 'error', 'timestamp'
                    ]
                    ...
                    for i, result in enumerate(self.results, 1):
                        ...
                        row = {
                            ...
                            'turn_2_input': result.get('turn_2_input', ''),
                            'turn_2_expected_response': result.get('turn_2_expected_response', ''),
                            'turn_2_actual_response': result.get('turn_2_actual_response', ''),
                            'turn_2_intent': result.get('turn_2_intent', ''),
                            'turn_2_actual_intent': result.get('turn_2_actual_intent', ''),
                            'session_id': result.get('session_id', ''),
                            'error': result.get('error', ''),
                            'timestamp': result.get('timestamp', '')
                        }
